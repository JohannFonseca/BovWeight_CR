                ...
                <main class="main-content">
                    <div class="page-header">
                        <div>
                            ...
                            <p class="page-subtitle">
                                Seguimiento de salud y peso de ganado autorizado
                            </p>
                        </div>

                        <ion-button class="primary-btn" @click="router.push('/veterinario/reportes')">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-details">
                                <span class="stat-value" v-if="!isLoading">{{ alertasPerdidaPeso }}</span>
                                <span class="stat-value" v-else>...</span>
                                <span class="stat-label">Alertas por pérdida de peso</span>
                            </div>
                        </div>
                    </div>

                    <h2 class="section-title-dashboard">
                        Resumen de Reportes Clínicos
                    </h2>
                        ...
                        <div class="panel-card animals-panel">
                            ...
                            <div class="panel-body no-padding">
                                <div class="animal-list">
                                    ...
                                    <div v-else-if="error" class="status-message warning-text">
                                        {{ error }}
                                    </div>

                                    <div v-else-if="seguimientoPrioritario.length === 0" class="status-message">
                                        No hay animales con pesajes recientes para seguimiento.
                                    </div>

                                    <template v-else>
                                        <div class="animal-row"
                                             v-for="animal in seguimientoPrioritario"
                                             :key="animal.id">
                                            <div class="animal-avatar"
                                                 :class="animal.tiene_alerta_perdida_peso ? 'warning' : 'normal'">
                                                <ion-icon :icon="animal.tiene_alerta_perdida_peso ? alertOutline : checkmarkOutline"></ion-icon>
                                            </div>

                                            <div class="animal-info">
                                                <h4 class="animal-name">
                                                    {{ animal.nombre }} #{{ animal.numero_arete }} ({{ animal.raza }})
                                                </h4>

                                                <span class="animal-tag">
                                                    Finca "{{ animal.finca }}" |
                                                    Último peso:
                                                    {{ animal.ultima_estimacion_kg ? animal.ultima_estimacion_kg + ' kg' : 'Sin registro' }}
                                                </span>

                                                <span v-if="animal.tiene_alerta_perdida_peso"
                                                      class="alert-detail">
                                                    Pérdida detectada:
                                                    {{ animal.porcentaje_perdida_peso }}%
                                                    en los últimos 30 días.
                                                    Peso anterior:
                                                    {{ animal.peso_referencia_kg }} kg.
                                                </span>

                                                <span v-else
                                                      class="normal-detail">
                                                    Sin pérdida mayor al 10% en los últimos 30 días.
                                                </span>
                                            </div>
...
<script setup lang="ts">
    ...
    const alertasPerdidaPeso = ref(0);
    ...
    const cargarSesion = () => {
        ...
        try {
            ...
            console.error('Error parseando usuario_sesion:', err);
            return null;
        }
    };
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            if (response.data && response.data.success) {
                ...
                alertasPerdidaPeso.value = data.alertas_perdida_peso || 0;
...
<style scoped>
    ...
    .stat-icon-wrapper {
        ...
        .stat-icon-wrapper.reports-total {
            background: #f4f6f0;
            color: #5c6e58;
        }

        .stat-icon-wrapper.reports-open {
            background: #ffebee;
            color: #c62828;
        }

        .stat-icon-wrapper.reports-progress {
            background: #fffde7;
            color: #f57f17;
        }

        .stat-icon-wrapper.reports-resolved {
            background: #e8f5e9;
            color: #2e7d32;
        }
    ...
    .animal-avatar {
        ...
        .animal-avatar.warning {
            background: #fff7ed;
            color: #d97706;
        }

        .animal-avatar.normal {
            background: #eaf0e6;
            color: #556b2f;
        }
